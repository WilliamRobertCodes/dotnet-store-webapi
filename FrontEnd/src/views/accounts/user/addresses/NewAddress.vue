...
const emit = defineEmits(['addressCreated', 'cancel']);
...
async function createAddress() {
    try {
        await api.post('accounts/addresses', { json: form }).json();
        emit('addressCreated');
    } catch (error) {
        if (!error.response) {
            throw error;
...
<template>
    <div>
        <h2 class="mb-4 text-xl font-bold">Add a new address</h2>
        <form @submit.prevent="createAddress" v-if="countries" class="space-y-4">
            ...
            <div class="flex justify-end gap-4 pt-3">
                ...
                <AppButton type="submit">Add address</AppButton>
            </div>
        </form>
    </div>
</template>
